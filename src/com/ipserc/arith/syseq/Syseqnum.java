...
public class Syseqnum extends MatrixComplex {
	 ...
	 * la generación de la documentación de la API -- principalmente congrad()/genminres(), que
	 * estaban enteros en español.
	 *
	 * 1.5 (2026_0807_1800)
	...
	/**
	 * Conjugate Gradient Method (CG): a Krylov iterative method to solve A*x=b, valid only when A
	 * is symmetric positive definite (if it is not, it falls back to triangleUp() as an
	 * approximation, but CG is not guaranteed to converge there -- see genminres() for the general
	 * case).
	 *
	 * At each iteration it builds a search direction p conjugate (A-orthogonal) to the previous
	 * ones, instead of a full Krylov basis as GMRES does -- so it does not need to store the whole
	 * basis nor solve a least-squares problem: each step has a closed formula (alpha, the optimal
	 * step size along p; the ratio ro1/ro2, how to blend the new direction with the previous one to
	 * preserve conjugacy). With exact arithmetic it converges in at most n steps (n = number of
	 * unknowns); in practice, with rounding, it may need more.
	 *
	 * @param initGuess The initial estimate, repeated across every component of the solution vector.
	 * @param iterations Maximum number of iterations.
	 * @return The solution found, or the best approximation reached if "iterations" is exhausted.
	 */
	public MatrixComplex congrad(Complex initGuess, int iterations) {
		final boolean DEBUG_ON = false;
		int k = -1;
	...
	/**
	 * Generalized Minimal Residual (GMRES): a Krylov iterative method to solve A*x=b without
	 * requiring A to be symmetric (unlike congrad()/CG, which does) -- restarted as GMRES(m).
	 *
	 * Via the Arnoldi process (modified Gram-Schmidt with a Hermitian inner product, needed because
	 * the matrices here can be complex), it builds an orthonormal basis V=[v1..vm] of the Krylov
	 * subspace generated by the initial residual, and, from the coefficients of that
	 * orthogonalization, a Hessenberg matrix H summarizing how A acts on that basis. Thanks to the
	 * orthonormality, minimizing the residual ||b-A*x|| within that subspace reduces to a small
	 * least-squares problem of only m unknowns: min||beta*e1-H*y||, solved via complex Givens
	 * rotations plus back-substitution. The solution is updated as x=x0+V*y; if that is not enough
	 * to converge, the Arnoldi process is restarted with the updated x as the new starting point, up
	 * to "iterations" restarts. "m" is the subspace size per restart: a larger value converges in
	 * fewer restarts but costs more per restart (memory/orthogonalization ~O(m^2)).
	 *
	 * @param initGuess The initial estimate, repeated across every component of the solution vector.
	 * @param m Dimension of the Krylov subspace at each restart (internally capped at the system size).
	 * @param iterations Maximum number of restarts.
	 * @return The solution found, or the best approximation reached if "iterations" is exhausted.
	 */
