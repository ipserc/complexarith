package com.ipserc.arith.fourier;
...
public class Fourier extends MatrixComplex {
	 ...
	 */
 	private static Complex integrate(Complex lolimit, Complex uplimit, Function <Complex, Complex> func, Function <Complex, Complex> expz, int numDec) {
		...
		if (((vectAngle >= Math.PI/4) && (vectAngle < 3*Math.PI/4 )) ||
			...
			return integrateIM(lolimit, uplimit, func, expz, precission);
		}
		else return integrateRE(lolimit, uplimit, func, expz, precission);
	}
	 ...
	 * @return The COMPLEX of the integral
	 */
	private static Complex integrateRE(Complex lolimit, Complex uplimit, Function <Complex, Complex> func, Function <Complex, Complex> expz, double precission) {
		Complex vector = uplimit.minus(lolimit);
	 ...
	 * @return The COMPLEX of the integral
	 */
	private static Complex integrateIM(Complex lolimit, Complex uplimit, Function <Complex, Complex> func, Function <Complex, Complex> expz, double precission) {
		Complex vector = uplimit.minus(lolimit);
		Complex nextPoint = new Complex();
		Complex integral = new Complex();
	...
	private Boolean isContinue(Complex p) {
		Complex fp, fp1;
		fp = func.apply(p);
		fp1 = func.apply(p.plus(Complex.getPrecision()));
		//fp.println("fp :");
		//fp1.println("fp1:");
	...
	private String getLineStyle(e_lineStyle lineStyle) {
	...
	public void plot(String title, int nbrSamples, MatrixComplex data, boolean showIm, e_lineStyle lineStyle) {
		...
		p.set("style", getLineStyle(lineStyle));
		p.set("grid","");
		p.plot();
	}
	 ...
	 */
	/**
	 * Does the sample of the function for the Fourier Series Analysis. The samples are stored in the Fourier Object
	 */
	public void doSrsSampling() {
		//this.N = sampleFreq;
		Complex incr = upLimit.minus(loLimit).divides(N);
		System.out.printf("Sample step: %e s\n", incr.rep());
		Complex point = loLimit;
	...
	/**
	 * Does the sample of the function for the Discrete Fourier Transform. The samples are stored in the Fourier Object
	 */
	 ...
	 * FREC. x axix represents the frequency associated to the coefficiente
	 */
	...
	*/
	 ...
	 * @param offset the offset for the first sample
