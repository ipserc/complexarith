...
public class TestMatrixOperators01 {
	...
	public static void main(String[] args) {
    	...
    	System.out.println();
  		...
  		//aMatrix = new MatrixComplex("3.0000E+00,5.0000E+00,6.0000E+00,2.0000E+00;-8.0000E+00,9.0000E+00,-3.0000E+00,-2.0000E+00;-1.0000E+00,3.0000E+00,6.0000E+00,-1.0000E+00;-1.0000E+00,7.0000E+00,2.0000E+00,5.0000E+00");
   		...
   		Complex.printBoxTitleRandom(boxSize, "MATRIX POWERS");
   		aMatrix.println("aMatrix");

		Complex.printBoxTitleRandom(boxSize, "POWER INT EXPONENT");
   		int iExp = 3;
   		System.out.println("Exponent:" + iExp);
		Complex.printBoxTextRandom(boxSize, "--- power(iExp) ---");
		bMatrix = aMatrix.power(iExp);
		bMatrix.println();
		Complex.printBoxTextRandom(boxSize, "--- ppower(iExp) ---");
		cMatrix = aMatrix.ppower(iExp);
		cMatrix.println();
		
		Complex.printBoxTitleRandom(boxSize, "POWER REAL EXPONENT");
   		double dExp = 3.5;
   		System.out.println("Exponent:" + dExp);
		Complex.printBoxTextRandom(boxSize, "--- power(dExp) ---");
		dMatrix = aMatrix.power(dExp);
		dMatrix.println();
		Complex.printBoxTextRandom(boxSize, "--- ppower(dExp) ---");
		eMatrix = aMatrix.ppower(dExp);
		eMatrix.println();

		Complex.printBoxTitleRandom(boxSize, "POWER COMPLEX EXPONENT");
   		Complex cExp = new Complex(3.4,-2.1);
   		cExp.println("Exponent:");
		Complex.printBoxTextRandom(boxSize, "--- power(cExp) ---");
		fMatrix = aMatrix.power(cExp);
		fMatrix.println();
		aMatrix.determinant().power(cExp).println("aMatrix.determinant().power(cExp):");
		fMatrix.determinant().println(            "fMatrix.determinant()............:");
		Complex.printBoxTextRandom(boxSize, "--- ppower(dExp) ---");
		gMatrix = aMatrix.ppower(cExp);
		gMatrix.println();
		aMatrix.determinant().power(cExp).println("aMatrix.determinant().power(cExp):");
		gMatrix.determinant().println(            "gMatrix.determinant()............:");
