...
public class ScratchMatrixComplexPlotAudit01 {
	...
	public static void main(String[] args) {
		...
		attempt("Fourier.plotSamples(int)", () -> f.plotSamplesSync("samples", 16, true, Fourier.e_lineStyle.LINES));
		attempt("Fourier.plotSeries", () -> f.plotSeriesSync("series", 16, true, Fourier.e_lineStyle.LINES));
		attempt("Fourier.plotSeries showIm=false", () -> f.plotSeriesSync("series-noim", 16, false, Fourier.e_lineStyle.LINES));
		attempt("Fourier.plotCompare", () -> f.plotCompareSync(16, Fourier.e_lineStyle.LINES));

		Fourier f2 = new Fourier(z -> Complex.sin(z.times(Complex.DOS_PI)), 0.0, 1.0);
		f2.DFT(16);
		attempt("Fourier.plotDFT SAMP", () -> f2.plotDFTSync("dft-samp", Fourier.e_domain.SAMP, Fourier.e_operator.COMPLEX, false, Fourier.e_lineStyle.LINES));
		attempt("Fourier.plotDFT FREC", () -> f2.plotDFTSync("dft-frec", Fourier.e_domain.FREC, Fourier.e_operator.MAGNITUDE, false, Fourier.e_lineStyle.LINES));

		// ---- Laplace ----
		Laplace l = new Laplace(z -> Complex.sin(z.times(Complex.DOS_PI)), new Complex(0, 0), new Complex(1, 0));
		l.DLT(16);
		attempt("Laplace.plotFunction", () -> l.plotFunctionSync("lfunc", 16, true, Laplace.e_lineStyle.LINES));
		attempt("Laplace.plotSamples", () -> l.plotSamplesSync("lsamples", true, Laplace.e_lineStyle.LINES));
		attempt("Laplace.plotDLT", () -> l.plotDLTSync("ldlt", true, Laplace.e_lineStyle.LINES));

		// ---- Z ----
		Z zt = new Z(z -> Complex.sin(z.times(Complex.DOS_PI)), new Complex(0, 0), new Complex(1, 0));
		zt.DZT(16);
		attempt("Z.plotFunction", () -> zt.plotFunctionSync("zfunc", 16, true, Z.e_lineStyle.LINES));
		attempt("Z.plotSamples", () -> zt.plotSamplesSync("zsamples", true, Z.e_lineStyle.LINES));
		attempt("Z.plotDZT", () -> zt.plotDZTSync("zdzt", true, Z.e_lineStyle.LINES));
