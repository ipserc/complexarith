...
public class TestMatrixOperators03 {
	...
	public static void main(String[] args) {
    	...
    	System.out.println();
   		...
   		aMatrix = new MatrixComplex("+7.2726E+01+9.8278E+01i,+7.5078E+00+5.4783E+01i,+1.6334E+01+6.4540E+01i;+8.8763E+01-7.5169E+01i,+9.8896E+01-3.3809E+01i,+5.2836E+01-2.1529E+01i;-9.3018E+01+1.1183E+00i,-1.7963E+01-2.6198E+01i,-1.0862E+01-3.8458E+01i");
		...
		Complex.printBoxTitleRandom(boxSize, "LOG NATURAL");
		Complex.printBoxTextRandom(boxSize, "--- log() ---");
		bMatrix = aMatrix.log();
		bMatrix.println();
		Complex.printBoxTextRandom(boxSize, "--- llog ---");
		cMatrix = aMatrix.llog();
		cMatrix.println();

		Complex.log(aMatrix.determinant()).println("log(aMatrix.determinant()).:");
		bMatrix.determinant().println(             "log(aMatrix).determinant().:");
		cMatrix.determinant().println(             "llog(aMatrix).determinant():");
		
		
		Complex.printBoxTitleRandom(boxSize, "LOG DECIMAL");
		Complex.printBoxTextRandom(boxSize, "--- log10() ---");
		dMatrix = aMatrix.log();
		dMatrix.println();
		Complex.printBoxTextRandom(boxSize, "--- llog10() ---");
		eMatrix = aMatrix.llog10();
		eMatrix.println();

		Complex.log10(aMatrix.determinant()).println("log10(aMatrix.determinant()).:");
		bMatrix.determinant().println(               "log10(aMatrix).determinant().:");
		cMatrix.determinant().println(               "llog10(aMatrix).determinant():");

		Complex.printBoxTitleRandom(boxSize, "LOG BASE DOUBLE");
		double dBase = 7.3;
		System.out.println("dBase: " + dBase);
		Complex.printBoxTextRandom(boxSize, "--- logbase(dBase) ---");
		dMatrix = aMatrix.logbase(dBase);
		dMatrix.println();
		Complex.printBoxTextRandom(boxSize, "--- llogbase(dBase) ---");
		eMatrix = aMatrix.llogbase(dBase);
		eMatrix.println();

		Complex.logbase(aMatrix.determinant(), dBase).println("logbase(aMatrix.determinant(), dBase).:");
		dMatrix.determinant().println(                        "logbase(aMatrix, dBase).determinant().:");
		eMatrix.determinant().println(                        "llogbase(aMatrix, dBase).determinant():");
