...
**Ya resueltos** (ver más abajo): bug de aliasing en `integrateRE`/`integrateIM`; `round(double,int)` usando la representación binaria exacta en vez de la decimal; `zeta_havil` sin criterio de convergencia; `binomialCoef(int,int)` inestable numéricamente; `gamma_weiertrass`/`gamma_euler` bug de `switch` + convergencia adaptativa.
...
Quedan pendientes de la auditoría, por orden sugerido: los 2 hallazgos de bajo riesgo/alto valor que quedan (`zeta_reflex` código muerto, `tan`/`cot`/`tanh`/`coth` recálculo doble), luego los de riesgo medio (empezando por `divides`, del que depende el de `arctan`/`arctanh`), y por último las 2 decisiones de alcance que requieren hablar con el usuario antes de tocar código. **Lección de proceso para las fases siguientes** (aprendida en los Fix 3 y 5): al usar un criterio de convergencia como "fix de bajo riesgo", verificar SIEMPRE (a) si la parada debe ser con igualdad exacta para garantizar resultado idéntico, o (b) si se acepta un cambio, que el umbral de convergencia esté ligado al parámetro de precisión que el propio método ya expone al llamador (`getMaxDecimals()`, `numDec`, etc.) y no a un ajuste global de la librería — de lo contrario se pierde silenciosamente el "mando" de precisión que el diseño original ya tenía, aunque fuera de forma implícita/aproximada (como pasó en el primer intento fallido de esta fase).

## PAUSA DE SESIÓN (30 julio 2026, ordenador hibernado por el usuario)

Estado en el momento de pausar: **todo commiteado**, `VERSION` en `1.22 (2026_0730_2102)`, último commit `28fb75e`. `git status` confirma que no hay ningún cambio pendiente en `src/com/ipserc/arith/complex/` ni en este documento — únicamente quedan sin trackear dos ficheros ajenos a esta tarea (`Claude/ClaudeRevisionComplexJava.txt`, `Claude/Revisiones/`), pre-existentes, nunca tocados en esta sesión.

**Al retomar, seguir con la auditoría matemática en este punto exacto**: siguiente hallazgo a abordar es `zeta_reflex` (código muerto en `ComplexFunctions.java`, candidato a eliminar con el mismo criterio que `zeta_riemann_siegel`/`zeta_analytic_continuation` de esta misma sesión — confirmar cero callers con grep antes de tocar nada, como se hizo entonces). Después: `tan`/`cot`/`tanh`/`coth` (recálculo doble, identidades de ángulo doble). Después, los de riesgo medio (`divides` primero, del que depende `arctan`/`arctanh`). Las 2 decisiones de alcance (`gamma_weiertrass`/`gamma_euler` como referencias lentas — nota: ya no están rotas tras el Fix 5, siguen siendo alternativas lentas legítimas; y la cuadratura adaptativa general) siguen pendientes de hablar con el usuario, no empezar sin preguntar.

Workflow vigente sin cambios: resumen breve antes de cada fix + confirmación, `Edit` únicamente, compilar, verificar con test suelto en el scratchpad contra un build de referencia fresco desde `HEAD`, batería de regresión (`TestComplex01/07`, `TestGamma01`, `TestZeta01`, más `TestGamma02-04` si el fix toca `gamma_weiertrass`/`gamma_euler`/`gamma_nemes`), bump de `VERSION` en cada commit que toque `Complex.java`, commit con mensaje largo, documentar en este fichero. Permiso amplio para pasos de verificación sin preguntar (concedido en esta sesión). Cuidado especial con criterios de convergencia (ver lección justo arriba).
