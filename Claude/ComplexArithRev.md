...
- `arctan`/`arctanh`/`acotan`/`acoth`: posible overflow análogo al ya corregido en `arcsin`/`arccos` en sesiones anteriores, vía `divides` — ahora que `divides` está arreglado (ver Fix 9 más abajo), revisar si esto sigue aplicando o ya quedó resuelto de rebote.
